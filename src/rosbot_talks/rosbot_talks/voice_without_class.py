...
"""Please perform the below operations on this command {command} generated by a human that involves direction and distance.
        Outputted direction should ONLY be within 'straight, left, right, or stop', and your response should ONLY be (direction by distance).
        If no distance is involved in the command, output distance as 0cm. 
        If there are directions like about turn, then output angle as 180. 
        For directions like backward, output direction should be 'straight', with distance in negative.
        If diagonal motion, then the output should be like turn left 15 degrees and move straight specified units.
...
def speech_to_text_callback():
    global flag, text_publisher, try_chat
    if flag:
